...
class IterRunner():
    def __init__(self, configs, train_loader, val_loader, eval_loader, model):
        ...
        self.model = model
        self.basis = torch.from_numpy(train_loader.dataset.faces).cuda()
    ...
    def train(self):
        ...
        zz, _ = self.model['head']['net'](feats)
        z = F.softmax(zz, dim=1)
        preds = torch.matmul(z, self.basis)
        loss = F.l1_loss(preds, faces)
        clc_loss = F.cross_entropy(zz, idx)

        # backward abd update model
        (0.000001 * loss + clc_loss).backward()
    ...
    def val(self,):
        ...
        for idx, voices, faces, _, _ in self.val_loader:
            ...
            loss += F.l1_loss(0.*preds, faces).item()
            count += voices.size(0)
